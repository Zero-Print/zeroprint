...
class ApiClient {
  ...
  async request<T>(
    ...
    try {
      ...
      // Handle API responses that follow the { success, data, message } format
      if (data && typeof data === 'object' && 'success' in data) {
        if (data.success) {
          ...
          throw new ApiError(data.error || 'API request failed', response.status, data.error, data);
        }
      }
  ...
  async get<T>(path: string, auth?: string): Promise<ApiResponse<T>> {
  ...
  monitoring = {
    getHealth: () =>
      this.get<SystemHealth>('/monitoring/health'),
    
    getAnalytics: (timeRange: TimeRange = '7d') =>
      this.get<Analytics>('/monitoring/analytics', { timeRange }),
    
    getPerformance: () =>
      this.get<PerformanceMetrics>('/monitoring/performance'),
    ...
    getAlerts: (params?: { page?: number; limit?: number; type?: string; severity?: string; resolved?: boolean }) =>
      this.get<SystemAlert[]>('/monitoring/alerts', params),
